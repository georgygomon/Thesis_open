fit_model_0<-function(data, N, n){
  ...
  failed<-0
  tryCatch(final_model_0<-inla(formula.model_0, family = c("gaussian","gaussian"),
                      data = final_data_0,verbose=FALSE,
                      control.compute = list(cpo = TRUE, dic = TRUE, waic = TRUE)),
           error = function(e) {print("INLA failed");failed<<-1})
  if (failed==1){
  ...
  return(list(coefficients=coefficients_0, mlik=final_model_0$mlik[1], dic_approx=-2*final_model_0$mlik[1], dic=final_model_0$dic$dic, 
              waic=final_model_0$waic$waic, pit=ks.test(final_model_0$cpo$pit,"punif",0,1)$statistic,
...
fit_model_1A<-function(data, N, n){
  ...
  N_obs<-length(data$x)
  fixed.effects<-list(Intercept1=c(rep(1, N_obs), rep(NA, N_obs)),
                      Intercept2=c(rep(NA, N_obs), rep(1, N_obs)),
                      x1=c(data$x, rep(NA, N_obs)),
                      x2=c(rep(NA, N_obs), data$x),
                      Period1=c(data$Period, rep(NA, N_obs)),
                      Period2=c(rep(NA, N_obs), data$Period))
  INLA_y<-list(c(data$y1, rep(NA, N_obs)),
               c(rep(NA, N_obs),data$y2))
  INLA_data<-c(fixed.effects)
  INLA_data$Y<-INLA_y
  INLA_data$i<-c(1:(2*N_obs))
  ...
  INLA_formula_1=Y~-1+f(Intercept1, model='linear')+f(Intercept2, model='linear')+
    ...
    f(i, model="iid4d", n=2*N_obs, hyper = list(theta1 = list(prior = "wishart4d")))
  failed<-0
  tryCatch(INLA_model_1<-inla(INLA_formula_1, family = c("gaussian","gaussian"),
  ...
  return(list(coefficients=coefficients_1, mlik=INLA_model_1$mlik[1], dic_approx=-2*INLA_model_1$mlik[1], dic=INLA_model_1$dic$dic, 
              waic=INLA_model_1$waic$waic,  pit=ks.test(INLA_model_1$cpo$pit,"punif",0,1),
...
fit_model_2A<-function(data, N, n){
  ...
  N_obs<-length(data$x)
  #Modelling as 2 separate likelihoods
  fixed.effects_2a<-list(Intercept1=c(rep(1, N_obs), rep(NA, N_obs)),
                         Intercept2=c(rep(NA, N_obs), rep(1, N_obs)),
                         x1=c(INLA_data_2a$x, rep(NA, N_obs)),
                         x2=c(rep(NA, N_obs), INLA_data_2a$x), 
                         Period1=c(INLA_data_2a$Period, rep(NA, N_obs)),
                         Period2=c(rep(NA, N_obs), INLA_data_2a$Period))
  
  random.effects_2a<-list(ID_all=c(INLA_data_2a$id, INLA_data_2a$id+N))
  y_final_2a<-list(c(INLA_data_2a$y1, rep(NA, N_obs)),
                   c(rep(NA, N_obs),INLA_data_2a$y2))
  ...
  failed<-0
  tryCatch(final_model_2a<-inla(formula.model_2a, family = c("gaussian","gaussian"),
  ...
  if (failed==1){
    ...
    return(list(coefficients=rep(NA,6), mlik=NA, dic_approx=NA, dic=NA, 
                waic=NA, pit=NA,
                cpo=NA, model_specific=list(cov_intercept=NA)))
  }
  ...
  tryCatch(SD_s<-sqrt(c(inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                        ...
                        inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                                      final_model_2a$internal.marginals.hyperpar[[4]]), silent=TRUE)$mean)), 
           error = function(e)
  ...
  return(list(coefficients=coefficients_2a, mlik=final_model_2a$mlik[1], dic_approx=-2*final_model_2a$mlik[1], dic=final_model_2a$dic$dic, 
              waic=final_model_2a$waic$waic, pit=ks.test(final_model_2a$cpo$pit,"punif",0,1)$statistic,
...
fit_model_2B<-function(data, N, n){
  ...
  INLA_data_2b<-data
  N_obs<-length(data$x)
  fixed.effects_2b<-list(Intercept1=c(rep(1, N_obs), rep(NA, N_obs)),
                         Intercept2=c(rep(NA, N_obs), rep(1, N_obs)),
                         x1=c(INLA_data_2b$x, rep(NA, N_obs)),
                         x2=c(rep(NA, N_obs), INLA_data_2b$x),
                         Period1=c(INLA_data_2b$Period, rep(NA, N_obs)),
                         Period2=c(rep(NA, N_obs), INLA_data_2b$Period))
  random.effects_2b<-list(ID_all=c(INLA_data_2b$id, INLA_data_2b$id+N))
  y_final_2b<-list(c(INLA_data_2b$y1, rep(NA, N_obs)),
                   c(rep(NA, N_obs),INLA_data_2b$y2))
  final_data_2b<-c(fixed.effects_2b, random.effects_2b)
  final_data_2b$Y<-y_final_2b
  final_data_2b$i<-c(1:(2*N_obs))
  ...
  #Run Model
  failed<-0
  tryCatch(final_model_2b<-inla(formula.model_2b, family = c("gaussian","gaussian"),
  ...
  if (failed==1){
    ...
    return(list(coefficients=rep(NA,6), mlik=NA, dic_approx=NA, dic=NA, 
                waic=NA, pit=NA,
                cpo=NA, model_specific=list(cov_intercept=NA, cov_errors=NA)))
  }
  ...
  tryCatch(SD_s<-sqrt(c(inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
          ...
          inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                        final_model_2b$internal.marginals.hyperpar[[2]]), silent=TRUE)$mean)), 
          error = function(e)
  ...
  SD_s<-diag(4)
  for (i in 1:4){
    tryCatch(SD_s[i,i]<-sqrt(inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                                           final_model_2b$internal.marginals.hyperpar[[(i+3)]]), silent=TRUE)$quant0.5),
  ...
  return(list(coefficients=coefficients_2b, mlik=final_model_2b$mlik[1], dic_approx=-2*final_model_2b$mlik[1], dic=final_model_2b$dic$dic, 
              waic=final_model_2b$waic$waic, pit=ks.test(final_model_2b$cpo$pit,"punif",0,1)$statistic,
...
fit_model_2C1<-function(data, N, n){
  ...
  N_obs<-length(data$x)
  #Modelling as 2 separate likelihoods
  Period_id<-rep(NA, times=N_obs)
  Period_id[INLA_data_2c1$Period2!=0]<-rep(1:N, each=(n-1))
  fixed.effects_2c1<-list(Intercept1=c(rep(1, N_obs), rep(NA, N_obs)),
                          Intercept2=c(rep(NA, N_obs), rep(1, N_obs)),
                          x1=c(INLA_data_2c1$x, rep(NA, N_obs)),
                          x2=c(rep(NA, N_obs), INLA_data_2c1$x), 
                          Period1=c(INLA_data_2c1$Period, rep(NA, N_obs)),
                          Period2=c(rep(NA, N_obs), INLA_data_2c1$Period),
  ...
  y_final_2c1<-list(c(INLA_data_2c1$y1, rep(NA, N_obs)),
                    c(rep(NA, N_obs),INLA_data_2c1$y2))
  ...
  failed<-0
  tryCatch(final_model_2c1<-inla(formula.model_2c1, family = c("gaussian","gaussian"),
  ...
  if (failed==1){
    ...
    return(list(coefficients=rep(NA,6), mlik=NA, dic_approx=NA, dic=NA, 
                waic=NA, pit=NA,
                cpo=NA, model_specific=list(cov_intercept=NA, cov_slope=NA)))
  }
  #Fixed effects
  ...
  tryCatch(SD_s<-sqrt(c(inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                        ...
                        inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                                      final_model_2c1$internal.marginals.hyperpar[[4]]), silent=TRUE)$mean)), 
           error = function(e)
  ...
  tryCatch(SD_s<-sqrt(c(inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
          ...
          inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                        final_model_2c1$internal.marginals.hyperpar[[7]]), silent=TRUE)$mean)),
          error = function(e)
            print("Model 2C1: Some covariance elements not well defined"))
  cor<-matrix(1, nrow=2, ncol=2)
  ...
  return(list(coefficients=coefficients_2c, mlik=final_model_2c1$mlik[1], dic_approx=-2*final_model_2c1$mlik[1], dic=final_model_2c1$dic$dic, 
              waic=final_model_2c1$waic$waic, pit=ks.test(final_model_2c1$cpo$pit,"punif",0,1)$statistic,
...
fit_model_2C2<-function(data, N, n){
  ...
  fixed.effects_2c2<-list(Intercept1=c(rep(1, N_obs), rep(NA, N_obs)),
                          Intercept2=c(rep(NA, N_obs), rep(1, N_obs)),
                          x1=c(INLA_data_2c2$x, rep(NA, N_obs)),
                          x2=c(rep(NA, N_obs), INLA_data_2c2$x), 
                          Period1=c(INLA_data_2c2$Period, rep(NA, N_obs)),
                          Period2=c(rep(NA, N_obs), INLA_data_2c2$Period),
  ...
  y_final_2c2<-list(c(INLA_data_2c2$y1, rep(NA, N_obs)),
                    c(rep(NA, N_obs),INLA_data_2c2$y2))
  ...
  #  f(ID_all_2,Period2, model="iid2d", n=2*N,hyper = list(theta1 = list(prior = "wishart2d", param =c(4,1,1, 0))))
  failed<-0
  tryCatch(final_model_2c2<-inla(formula.model_2c2, family = c("gaussian","gaussian"),
  ...
  if (failed==1){
    ...
    return(list(coefficients=rep(NA,6), mlik=NA, dic_approx=NA, dic=NA, 
                waic=NA, pit=NA,
                cpo=NA, model_specific=list(cov_both=NA)))
  }
  ...
  tryCatch(SD_s<-sqrt(c(inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
               ...
               inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                             final_model_2c2$internal.marginals.hyperpar[[4]]), silent=TRUE)$mean,
               inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                             final_model_2c2$internal.marginals.hyperpar[[5]]), silent=TRUE)$mean,
               inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                             final_model_2c2$internal.marginals.hyperpar[[6]]), silent=TRUE)$mean)), 
           error = function(e)
  ...
  return(list(coefficients=coefficients_2c2, mlik=final_model_2c2$mlik[1], dic_approx=-2*final_model_2c2$mlik[1], dic=final_model_2c2$dic$dic, 
              waic=final_model_2c2$waic$waic, pit=ks.test(final_model_2c2$cpo$pit,"punif",0,1)$statistic,
...
fit_model_3A1<-function(data, N, n){
  ...
  failed<-0
  tryCatch(final_model_3a<-inla(formula.model_3a, family = c("gaussian","gaussian"),
  ...
  if (failed==1){
    ...
    return(list(coefficients=rep(NA,6), mlik=NA, dic_approx=NA, dic=NA, 
                waic=NA, pit=NA,
                cpo=NA, model_specific=list(cov_both_1=NA, cov_both_2=NA, 
                                            model_coefficients=NA, gamma=NA)))
  }
  ...
  return(list(coefficients=actual_coefficients, mlik=final_model_3a$mlik[1], dic_approx=-2*final_model_3a$mlik[1], dic=final_model_3a$dic$dic, 
              waic=final_model_3a$waic$waic,  pit=ks.test(final_model_3a$cpo$pit,"punif",0,1)$statistic,
...
fit_model_3A2<-function(data, N, n){
  INLA_data_3a2<-data
  #Modelling as 2 separate likelihoods
  ...
  failed<-0
  tryCatch(final_model_3a2<-inla(formula.model_3a2, family = c("gaussian","gaussian", "gaussian"),
                        data = final_data_3a2,verbose=FALSE, control.compute = list(cpo = TRUE, dic = TRUE, waic = TRUE),
  ...
  model_coefficients<-final_model_3a2$summary.fixed[,1]
  actual_coefficients<-c(model_coefficients[1:3], model_coefficients[1:3]*gamma+model_coefficients[4:6])
  ...
  tryCatch(SD_s<-sqrt(c(inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                        ...
                        inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                                      final_model_3a$internal.marginals.hyperpar[[4]]), silent=TRUE)$mean)),
  ...
  return(list(coefficients=actual_coefficients, mlik=final_model_3a2$mlik[1], dic_approx=-2*final_model_3a2$mlik[1], dic=final_model_3a2$dic$dic, 
              waic=final_model_3a2$waic$waic, pit=ks.test(final_model_3a2$cpo$pit,"punif",0,1)$statistic,
              cpo=-sum(log(final_model_3a2$cpo$cpo)), model_specific=list(cov_both_1=Cov1_0_t, cov_both_2=Cov2_0_t, 
                                                                         model_coefficients=model_coefficients, gamma=gamma)))
}
...
fit_model_3B1<-function(data, N, n){
  ...
  fixed.effects_3b1<-list(x=c(INLA_data_3b1$x, INLA_data_3b1$x), 
                          ...
                          Intercept2=c(rep(NA, N_obs), rep(1, N_obs)),
                          Intercept1=c(rep(1, N_obs), rep(NA, N_obs)),
                          Period2=c(rep(NA, N_obs),INLA_data_3b1$Period),
                          Period1=c(INLA_data_3b1$Period, rep(NA, N_obs)),
                          x2=c(rep(NA, N_obs),INLA_data_3b1$x),
                          x1=c(INLA_data_3b1$x, rep(NA, N_obs)))
  random.effects_3b1<-list(Intercept_random1=c(INLA_data_3b1$id, rep(NA, N_obs)),
                           Intercept_random12=c(rep(NA, N_obs), INLA_data_3b1$id),
                           Intercept_random2=c(rep(NA, N_obs), INLA_data_3b1$id),
                           Period_random1=c(INLA_data_3b1$id, rep(NA, N_obs)),
                           Period_random12=c(rep(NA, N_obs), INLA_data_3b1$id),
                           Period_random2=c(rep(NA, N_obs), INLA_data_3b1$id+N))
  y_final_3b1<-list(c(INLA_data_3b1$y1, rep(NA, N_obs)),
                    c(rep(NA, N_obs),INLA_data_3b1$y2))
  ...
  failed<-0
  tryCatch(final_model_3b1<-inla(formula.model_3b1, family = c("gaussian","gaussian"),
  ...
  return(list(coefficients=coefficients_3b1, mlik=final_model_3b1$mlik[1], dic_approx=-2*final_model_3b1$mlik[1], dic=final_model_3b1$dic$dic, 
              waic=final_model_3b1$waic$waic, pit=ks.test(final_model_3b1$cpo$pit,"punif",0,1)$statistic,
