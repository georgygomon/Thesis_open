...
################################################################################
#Model 0: Independent longitudinal models with per outcome dependent Random Slope & Intercept
################################################################################
set.seed(2022)
...
final_model_0<-inla(formula.model_0, family = c("gaussian","gaussian"),
                     data = final_data_0,verbose=TRUE)

summary(final_model_0)
coefficients_0$coefficients$INLA<-final_model_0$summary.fixed[,1]


names(final_model_0$internal.marginals.hyperpar)
SD_s<-diag(2)
...
SD_s<-diag(2)
tryCatch(SD_s<-sqrt(c(inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
                                                    final_model_0$internal.marginals.hyperpar[[6]]))$mean,
                      inla.zmarginal(inla.tmarginal(function(x) 1/exp(x),
